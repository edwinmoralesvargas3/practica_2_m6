    ...
    <div class="container  ">
        ...
        <div class="row">
            <div class="flex-container">
                <div v-for="p in productosRelacionados" v-bind:key="p">
                    <div class="card" style="width: 18rem;">
                        <div class="card-body">
                            <h5 class="card-title">{{ p.nombre }}</h5>
                            <img v-bind:src="p.imagen" alt="im" :style="{ width: '100%' }">
                            <p class="card-text">{{ p.descripcion }}</p>
                            <div class="producto-relacionado-precio">Precio: {{ p.precio }} BOB</div>
                            <div class="flex-container">
                                <div v-for="c in p.colores" v-bind:key="c">
                                    <div class="color-box" v-bind:style="{ backgroundColor: c }"></div>
                                </div>
                            </div>
                        </div>
                    </div>
